...
var Profile = function Profile(bucketName, f) {
  ...
  return function () {
    ...
    var timer = new Timer(JSON.stringify(currentEntry), function (durationMs) {
    ...
    try {
      ...
      if (Fiber.current) {
        ...
        if (timer !== poppedTimer) {
          throw new Error("unexpected timer at top of stack: " + poppedTimer.id + "; expected: " + timer.id);
        }
      }
      currentEntry.pop();
    }
...
var runContinuously = function runContinuously() {
  if (!enabled) {
    return;
  }
